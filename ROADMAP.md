...
状态：实施中。范围仅为 Arm Preview 和腕部视图；保持后端 API、模型包格式、动作来源与虚拟 follower 行为。

架构：保留 Three.js WebGLRenderer，将相机、关节动画与腕部刷新集中到单一按需调度器；主相机无阻尼最高 60 FPS，姿态插值最高 30 FPS，腕部最高 10 FPS。两个视口独立判断可见性，静止时停止绘制。

基线：现有 force 路径绕过限频；controls.update/change 可重复排队；腕部被强制提频；抗锯齿关闭、DPR 上限 1.25；内置外壳为黄色 Phong 材质。硬件 GPU 基线尚未采集，不预先声称改进幅度。

里程碑：
1. [ ] 修复调度、姿态收敛、可见性与异步模型生命周期；增加确定性行为测试。
2. [ ] 内置模型采用共享冷灰/深灰 Lambert 材质、中性双灯、原生抗锯齿（主 DPR ≤1.5，腕部 320×180 / DPR 1）；按钮右下角 8px、半透明。
3. [ ] 浏览器回归、DPR/布局截图、性能对比，更新 dev_log 并分阶段提交。
